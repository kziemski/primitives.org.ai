 ...
 *
 * Note: For .map() batch processing, use the full primitives from ai-functions.
 *
 * @example
 * ```ts
...
export function decide(
  ...
  return <T>(...options: T[]): AIPromise<T> => {
    ...
    ;(promise as unknown as { resolve: () => Promise<T> }).resolve = async () => {
      const result = await originalResolve() as unknown as { chosenIndex: string | number }
